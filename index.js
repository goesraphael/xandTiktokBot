...
async function main() {
  ...
  // Simula envio ao TikTok
  console.log("🔄 [Simulação] Vídeo gerado e pronto para postar no TikTok:", videoPath);
}
...
function createVideoWithFFmpeg(imagePath, audioPath, outputPath) {
  execSync(`ffmpeg -y -loop 1 -i ${imagePath} -i ${audioPath} -c:v libx264 -tune stillimage -t 15 -pix_fmt yuv420p -vf "zoompan=z='min(zoom+0.0005,1.5)':d=125" -shortest ${outputPath}`);
}
